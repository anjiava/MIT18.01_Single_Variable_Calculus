...
Well, a tangent line has an equation, and any line through a point has the equation y - y_0 is equal to m, the slope, times x - x_0.
...
it's going to be y - y_0 is equal to, and here's the magic number, we already calculated it.
...
We know that y0 is f(x_0) , and f(x) is 1/x , so this thing is 1/x_0.
...
And this is plus 1/x_0 because the x_0 and x0^2 cancel somewhat.
...
So it's 1/2 (2x_0) * (2y_0) , which is 2x_0 y_0, which is, lo and behold, 2.
...
And previously we wrote f prime for the derivative, so this is Newton's notation for the derivative.
...
We have delta f / delta x, which remember was 1 / delta x times this, which is this times, now this is x^n plus nx^(n-1) delta x plus this junk term, minus x^n.
